const officeTopicListUrl = '/backoffice/mainoffice/office/interesting/easy-english';
const officeTopicPage = new URLSearchParams(window.location.search).get('page') || '1';

fetch(`/api/backoffice/office/topics/03-10?page=${encodeURIComponent(officeTopicPage)}`, {
  credentials: 'include'
        ...
        if (data.data.topics.length >= 1) {
            data.data.topics.forEach(element => {
                ...
                clone.querySelector('[data-role="toggle-link"]').addEventListener('click', event => {
                    ...
                    fetch(`/api/backoffice/office/topics/03-10/${encodeURIComponent(element.topic_id)}/toggle-active`, {
                        method: 'POST',
                        credentials: 'include'
                ...
                if (data.data.isAdmin) {
                    clone.querySelector('[data-role="delete-link"]').addEventListener('click', () => {
                        if (confirm('Are you sure ? want delete this topic ?')) {
                            fetch(`/api/backoffice/office/topics/03-10/${encodeURIComponent(element.topic_id)}`, {
                                method: 'DELETE',
                                credentials: 'include'
