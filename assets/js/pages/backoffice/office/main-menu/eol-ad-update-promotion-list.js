const mainMenuListUrl = '/backoffice/mainoffice/office/main-menu/eol-ad-update-promotion';
const mainMenuPage = new URLSearchParams(window.location.search).get('page') || '1';

fetch(`/api/backoffice/office/topics/01-07?page=${encodeURIComponent(mainMenuPage)}`, {
  credentials: 'include'
    ...
    if (data.status === 200) {
        ...
        document.getElementById('office-topic-add-link').href = `${mainMenuListUrl}/add`
        ...
        for (let i = 1; i <= data.data.allPages; i++) {
            ...
            pageClone.querySelector('[data-role="page-link"]').href = `${mainMenuListUrl}?page=${i}`
        ...
        if (data.data.topics.length >= 1) {
            data.data.topics.forEach(element => {
                ...
                clone.querySelector('[data-role="detail-link"]').href = `${mainMenuListUrl}/${element.topic_id}/detail?page=${data.data.page}`
                clone.querySelector('[data-role="toggle-link"]').addEventListener('click', event => {
                    event.preventDefault()
                    fetch(`/api/backoffice/office/topics/01-07/${encodeURIComponent(element.topic_id)}/toggle-active`, {
                        method: 'POST',
                        credentials: 'include'
                ...
                clone.querySelector('[data-role="edit-link"]').href = `${mainMenuListUrl}/${element.topic_id}/edit?page=${data.data.page}`
                if (data.data.isAdmin) {
                    clone.querySelector('[data-role="delete-link"]').addEventListener('click', () => {
                        if (confirm('Are you sure ? want delete this topic ?')) {
                            fetch(`/api/backoffice/office/topics/01-07/${encodeURIComponent(element.topic_id)}`, {
                                method: 'DELETE',
                                credentials: 'include'
